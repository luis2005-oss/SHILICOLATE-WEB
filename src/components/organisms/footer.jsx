...
function Footer() {
  return (
    <footer
      className="bg-[#120804] border-t border-[#c8895a22]"
      style={{ fontFamily: "'Inter', sans-serif" }}
    >
      <div className="max-w-[1300px] mx-auto px-6 pt-16 pb-0">
        {/* GRID */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.6fr_1fr_1.3fr_1fr] gap-10 pb-12">
          {/* Marca */}
          <div className="flex flex-col gap-5">
            ...
            <p className="font-light text-[0.88rem] text-[#7a5c45] leading-[1.7] max-w-[240px]">
              Chocolatería artesanal nacida en Celendín. Cada pieza elaborada con
              cacao selecto y amor por nuestra cultura.
            </p>
          ...
          <FooterCol title="Productos">
            <FooterSubTitle>Chocolate al 100%</FooterSubTitle>

            {chocolate100.map(({ text, href }) => (
              <Link
                key={href}
                to={href}
                className="text-[#c4a98a] py-1 block hover:text-[#f5e8d5] text-[0.88rem]"
              >
                {text}
              </Link>
            ))}

            <FooterSubTitle>Comestibles</FooterSubTitle>

            {comestibles.map(({ text, href }) => (
              <Link
                key={href}
                to={href}
                className="text-[#c4a98a] py-1 block hover:text-[#f5e8d5] text-[0.88rem]"
            ...
            {contactItems.map((item) => {
              ...
              return (
                <div key={item.text} className="flex items-start gap-2 py-1.5">
                  <Icon size={12} className="text-[#c8895a] mt-1" />
